...
points_4 = np.linspace(0, 0.47, sparce)
points_5 = np.linspace(0.47 + 1/(100*(a_pts - sparce)) , 0.5, a_pts - sparce)
points_6 = np.linspace(0.5 + 1/(100*(b_pts - sparce)), 0.53, b_pts - sparce)
points_7 = np.linspace(0.53 + 1/50 , 1, sparce)
...
ax1.plot(points, prediction, label = 'WE-GE-PINN solution with RAR input', color='green')
...
for point in range(1, a_pts - 1, 1):
    ...
    #L2 residual
    #residual += float(1/len(points))* (np.square((exact_2[point]-predictions_b_1[point])/exact_2[point]) + np.square((exact_1[point]-predictions_a_1[point])/exact_1[point]))
