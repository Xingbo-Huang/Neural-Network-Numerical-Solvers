...
def compute_loss_CAE():
    ...
    loss_total = 5 * loss_linear_uncorrelation + loss_gaussianality + 5 * loss_nonlinear_uncorrelation + 0.05 * loss_reconstruction
...
def compute_loss_Modes():

    Init = tf.cast(tf.convert_to_tensor(np.array([ [1, np.sqrt(2), 1]]).reshape(3, 1)), tf.float32)
    ...
    W_3 = model_Decoder.get_layer('output').weights[0]

    loss_IC =  tf.reduce_mean(tf.square(tf.transpose(W_3) - Init))

    loss_total = loss_reconstruction + loss_IC

    dummy1 = 0
    dummy2 = 0
    dummy3 = 0

    return loss_total, dummy1, dummy2, dummy3, W_3, X_hat
...
def get_grad_Modes():
    ...
    with tf.GradientTape(persistent=True) as tape:
        ...
        loss, u_xa, ua, s, Q, X_hat = compute_loss_Modes()
...
@tf.function
def train_step_Modes():

    #print("tran_step() is working")

    lossa, grad_theta_a, u_xa, ua, s, Q, X_hat= get_grad_Modes()
...
for k in range(k_n):
    ...
    for i in range(L):
        ...
        loss_Modes, u_x_CAE, u_CAE, s, Q, X_hat = train_step_Modes()
    
        if num_epoch_1%5 == 0 :
        
            print("Epoch: ", num_epoch_1)

        epoch_list_1.append(num_epoch_1)
        loss_list_1.append(loss_Modes)
    ...
    for i in range(M):
        ...
        if num_epoch_2%5 == 0 :
            ...
            print("Epoch: ", num_epoch_2)

        epoch_list_2.append(num_epoch_2)
        loss_list_2.append(loss_CAE)
...
ax1.grid()
ax1.legend()
ax2.plot(epoch_list_1, loss_list_1, label = "Loss Histroy", color = "red")
ax2.grid()
ax2.legend()
