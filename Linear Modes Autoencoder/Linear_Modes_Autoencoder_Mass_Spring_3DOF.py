...
def init_Encoder_model(input_dim, latent_dim, output_dim):
    ...
    model.add(tf.keras.layers.Dense(input_dim, activation=tf.keras.activations.get('sigmoid'), name = "hidden_1"
...
def compute_loss_CAE():
    ...
    loss_linear_uncorrelation = tf.reduce_max(tf.reduce_sum(tf_cov(H) - tf.eye(num_input)))
...
file_path = r"C:\Xingbo\University\McGill\Summer 2023\Research\Legrand\Code\Linear Modes Autoencoder\Modal Response data\3DOF.text"
