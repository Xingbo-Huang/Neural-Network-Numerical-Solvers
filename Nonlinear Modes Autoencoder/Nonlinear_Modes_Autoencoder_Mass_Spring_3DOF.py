...
text_file_path = r"C:\Xingbo\University\McGill\Summer 2023\Research\Legrand\Code\Training data\Mass_Spring\Mass_Spring_Data_3DOF_Nonlinear.txt"
...
def init_Encoder_model(input_dim, latent_dim, output_dim):
    ...
    model.add(tf.keras.layers.Dense(input_dim, activation=tf.keras.activations.get('tanh'), name = "hidden_1", use_bias= False))

    # Output layer
    model.add(tf.keras.layers.Dense(latent_dim, name = "Encoder_output"                               
    ,use_bias= False
...
def init_Decoder_model(input_dim, latent_dim, output_dim):
    ...
    # Output layer
    model.add(tf.keras.layers.Dense((output_dim), activation=tf.keras.activations.get('tanh'), name = "decoder_hidden", use_bias= False))
...
def compute_loss_CAE():
    ...
    loss_total = 0 * loss_linear_uncorrelation + 0 * loss_gaussianality + 55 * loss_nonlinear_uncorrelation + 0.5 * loss_reconstruction
...
optim_CAE = tf.keras.optimizers.Adam(learning_rate=0.008)
...
for i in range(0, len(t) - 1):
    ...
    for j in range(1):
        ...
        Q_select_diff = Q_select[i+1] - Q_select[i]
        q_diff_list_1.append(Q_select_diff)

Q_dot = q_diff_list_1

Spring_Energy = 0.5 * 1 * tf.square(Q[:, 0])
Kinetic_Energy = 0.5 * 1 * tf.square(tf.convert_to_tensor(Q_dot))

Energy_total = Kinetic_Energy

fig, (ax15) = plt.subplots(1,1, figsize=(12,6))

ax15.plot(t[0:999], Energy_total, color = "blue", label = "Energy 1")
ax15.set(xlabel = "time(s)", ylabel = "E(J)")
ax15.grid()
...
# fig = plt.figure()
# ax16 = fig.add_subplot(111, projection='3d')

# Q1, Q2 = np.meshgrid(Q[:, 0].numpy()[0:999], Q[:, 0].numpy()[0:999])

# # Plot the surface
# surface = ax16.plot_surface(Q1, Q2, np.array(Q_dot), cmap='viridis')
...
fig, (ax9, ax10, ax11) = plt.subplots(1,3,figsize=(18,6))

ax9.plot(Q[:, 0], Q[:, 1], color = "blue", label = "Modal Response 1")
ax10.plot(Q[:, 0], Q[:, 2], color = "red", label = "Modal Response 2")
ax11.plot(Q[:, 1], Q[:, 2], color = "orange", label = "Modal Response 3")
ax9.set(xlabel = "q1", ylabel = "q2")
ax9.grid()
ax10.set(xlabel = "q1", ylabel = "q3")
ax10.grid()
ax11.set(xlabel = "q2", ylabel = "q3")
ax11.grid()
...
fig, (ax12, ax13, ax14) = plt.subplots(1,3,figsize=(18,6))

ax12.plot(X_hat[:, 0], X_hat[:, 1], color = "blue", label = "X1 vs X2")
ax13.plot(X_hat[:, 0], X_hat[:, 2], color = "red", label = "X2 vs X3")
ax14.plot(X_hat[:, 1], X_hat[:, 2], color = "orange", label = "X2 vs X3")
ax12.set(xlabel = "x1", ylabel = "x2")
ax13.grid()
ax14.set(xlabel = "x1", ylabel = "x3")
ax12.grid()
ax13.set(xlabel = "x2", ylabel = "x3")
ax14.grid()
...
file_path = r"C:\Xingbo\University\McGill\Summer 2023\Research\Legrand\Code\Linear Modes Autoencoder\Modal Response data\3DOF_Original_Nonlinear.text"
...
file_path = r"C:\Xingbo\University\McGill\Summer 2023\Research\Legrand\Code\Linear Modes Autoencoder\Modal Response data\3DOF_Energy.text"
...
for j in range(1):
    for i in range(len(Energy_total)):
        ...
        E_string.append(str(Energy_total[i].numpy()))

    E_soln_string = ", ".join(E_string)

    E_string = []
    file.write(E_soln_string)
...
ax1.plot(x, W[2], label = 'Linear Mode 3', color='blue', marker = "o")
...
ax7.plot(t, x_data_tensor[2, :], color = "blue", label = "X3")
ax7.plot(t, X_hat[:, 2], color = "green", label = "X hat 3")
